...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Use app-server `account/usage/read` for account-token comparison | June 13, 2026 `codex app-server generate-ts --out` exposes `account/usage/read` returning `GetAccountTokenUsageResponse`: lifetime tokens, peak daily tokens, longest running turn seconds, current/longest streak days, and daily token buckets. Current app still fetches Profile tokens through direct `/wham/profiles/me` HTTP after `getAuthStatus`. | `CodexAppServerClient.fetchProfileTokenUsageSnapshot()` and `CodexProfileTokenUsageHTTPClient` in `CodexRateLimitModels.swift`; Settings Data Profile comparison currently stores only Profile lifetime/peak/buckets. | Add an app-server account-usage fetch/decoder path, prefer it for Settings Data account-token comparison, keep the existing HTTP Profile path only as a compatibility fallback if needed, and add safe Settings rows for longest turn and streak metrics. Preserve local captured token semantics and do not store account identity/auth/raw response. | Decoder/client tests for `account/usage/read`, fallback tests, Settings Data state tests, `git diff --check`, full Xcode tests, push, `./install.sh`, process/fingerprint/visible Settings verification. | Yes; requirements/spec/plan archived in `archive/2026-06-13-app-server-account-usage-read.md`. |
| 2 | Extend app-server notification audit for Codex 0.140 v2 surfaces | June 13 generated `ServerNotification` includes additional v2 methods beyond the current sanitizer coverage, including hook started/completed, app list updated, external-agent import completed, fs changed, MCP server startup status, item lifecycle, raw response item completed, plan/reasoning deltas, process/command/file-change deltas, and account login completed. | `CodexAppServerNotificationAuditSanitizer` currently supports a narrower set and treats unknown methods as unsupported count-only. | Expand audit v2 to count and sanitize new 0.140 methods using strict enum/presence/count-only rules. Keep content-like streams as counts/presence only; no popover warnings for ordinary audit data. | Sanitizer privacy tests for each new method family, old JSON decode tests, Settings Data audit rendering tests, full Xcode tests, install/relaunch visual check. | Yes. |
| 3 | Add local source freshness and capture coverage diagnostics | June 13 live evidence: `logs_2.sqlite` has 352,892 rows and 1,000 today rows; app DB has 93,912 token samples, 295,768 turn-performance events, 120,854 runtime dimensions, 1,585 task timing rows, 490 thread catalog rows, and 5 model capabilities. Runtime dimensions are populated but latest dimension catalog seen time is June 7; app-server diagnostics are connected with 184 inbound notifications but `0` token usage notifications and `0` notification-audit method rows. | Capture state tables show per-import status but Settings does not clearly distinguish source availability, matched-row coverage, latest source row, latest stored event, and passive app-server no-sample states. | Add a compact Settings Data source-coverage summary: source exists, source schema recognized, latest source event, latest stored event, rows scanned/matched where available, and "connected but no samples" states for passive app-server audits. Keep values aggregate-only. | Store/read helper tests, Settings Data tests for healthy/stale/no-match/error states, safe live verification with counts only, full Xcode tests, install/relaunch visible check. | Yes. |
| 4 | Add generated app-server surface drift check | Advisory audit found the app-server notification sanitizer is intentionally allowlist-based, but Codex 0.140 exposes many generated methods. Manual audits can miss drift between generated protocol and sanitizer coverage. | `CodexAppServerNotificationAuditSanitizer.supportedMethods` and skipped methods are hand-maintained; there is no test or script comparing them to generated `ServerNotification`. | Add a repo-local diagnostic/test helper that runs or fixtures `codex app-server generate-ts --out`, extracts server notification method names, and reports newly unsupported methods as count-only candidates without failing routine builds unless an accepted fixture is updated. | Unit/script tests on fixture, privacy review that only method names are recorded, `git diff --check`, full Xcode tests, install/relaunch if app code changes. | Yes. |
| 5 | Tighten source-health update metadata and redacted diagnostics paths | Advisory audit noted `version.json.dismissed_version` is ignored, and Settings/source-health diagnostics can expose full local paths. This is useful locally but should be explicit and redacted in exports or shareable diagnostics. | `CodexSourceHealthReader`, `CodexSourceHealthSnapshot`, Settings Data source-health rendering/export paths, and diagnostics JSON models. | Capture/display dismissed-version state, distinguish stale update metadata from intentionally dismissed updates, and add redacted/export-safe variants for path-bearing diagnostics. Keep full local paths only where they are clearly local-only and useful. | Reader/store tests for dismissed version, Settings tests for dismissed/stale states, export/privacy tests for path redaction, full Xcode tests, install/relaunch visible Settings check. | Yes. |

## Codex Update Audit Details

- June 13, 2026 local audit evidence:
  - App-bundled Codex binary reports `codex-cli 0.140.0-alpha.2`; `~/.codex/models_cache.json` reports `client_version = 0.140.0` and contains 5 models.
  - Homebrew `/opt/homebrew/bin/codex` still reports `codex-cli 0.128.0`; `~/.codex/version.json` still reports `latest_version = 0.128.0` with `last_checked_at = 2026-05-05T02:25:24.357406Z`.
  - `logs_2.sqlite.logs` uses `ts` and `feedback_log_body`; current app code already queries those columns.
  - `logs_2.sqlite` safe aggregate counts: 352,892 log rows, 1,000 rows today, 1,586 response-completed-like rows, 246,308 token-key-like rows, 236,962 OTEL-runtime-key-like rows, and 659 task-key-like rows.
  - `state_5.sqlite` has 490 threads, 383 spawn edges, 407 dynamic tools, 1 remote-control enrollment, and empty `agent_jobs` / `agent_job_items` tables.
  - App history database safe aggregate counts: 93,912 token samples, 295,768 turn-performance events, 120,854 turn-performance runtime dimensions, 1,585 session task timing rows, 490 thread catalog rows, and 5 model capability rows.
  - Recent active session JSONL remains rich: 172 files modified in the last 7 days, with high counts for `token_count`, `task_started`, `task_complete`, tool-call, and response item event types by key presence.
  - `codex app-server generate-ts` now requires `--out <DIR>` and exposes a broader v2 protocol. Notable safe/product-relevant surfaces include `account/usage/read`, `account/rateLimits/read` with `rateLimitsByLimitId`, `modelProvider/capabilities/read`, `permissionProfile/list`, app/plugin/skill/MCP/hook requests, and many additional notification methods.
  - Persisted app-server diagnostics are connected over WebSocket and have received inbound notifications, but passive `thread/tokenUsage/updated` and notification-audit method samples remain absent in the current installed app state.
  - Product direction: prefer first-party app-server request paths over direct backend HTTP where they expose the same or better sanitized account metadata, and keep passive notification/log surfaces diagnostic-first until real captured samples justify UI features.
...
- Watch Codex agent-job tables
  - `state_5.sqlite` now contains `agent_jobs` and `agent_job_items`, but both tables had `0` rows during both the May 30 and June 13 audits.
...
- Post-Codex 0.140 update audit and backlog update
  - Ran an owner-side, privacy-preserving audit of local Codex 0.140 sources and current app integration using schemas, counts, key names, generated protocol types, and persisted diagnostic summaries only.
  - Initialized the same-account handoff at `~/.codex/handoffs/codex-status-bar` for `worker_5.5`; direct thread messaging was not available in the owner session, so worker preflight remains a manual bootstrap step.
  - Added prioritized Remaining Work for account usage, app-server notification audit v2 expansion, and local source freshness/coverage diagnostics.
  - Archived requirements/spec/plan for the first recommended implementation item.
