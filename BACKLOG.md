...
- Add local source freshness and capture coverage diagnostics
  - Added an aggregate-only local source coverage snapshot for Settings > Data covering Codex logs, session task timing, state metadata, model capabilities, remote-control enrollment health, live token notifications, and notification-audit diagnostics.
  - Added safe source probes for file/table/schema presence and bounded counts/latest timestamps, plus read-only stored-row metrics from app-owned tables.
  - Surfaced explicit fresh, stale, source missing, schema missing, no matched rows, connected passive no-sample, failed, and not-checked states while keeping existing detailed capture sections.
  - Preserved privacy boundaries by showing only counts, timestamps, status labels, and generic error-presence summaries, excluding raw source rows, prompts/messages, thread/turn IDs, URLs, local paths, auth/account/user identifiers, and arbitrary JSON values.
  - Added focused store/read, source-probe, Settings view-model, stale-state, passive app-server no-sample, and redaction tests.
  - Next recommended item: `Add generated app-server surface drift check`.
