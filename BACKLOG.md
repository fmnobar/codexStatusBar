...
- Extend app-server notification audit for Codex 0.140 v2 surfaces
  - Expanded the existing JSON-backed notification audit sanitizer to recognize the new Codex 0.140 server notification families: thread lifecycle/name/compaction, hooks, item lifecycle and approval review, raw response items, streaming deltas, command/process/file-change updates, server/MCP/app metadata, filesystem/fuzzy search, Windows setup warnings, and account login.
  - Kept `account/rateLimits/updated`, `remoteControl/status/changed`, and `thread/tokenUsage/updated` on their existing specialized diagnostic paths.
  - Stored only safe enum/status/kind values, model-normalized fields where explicitly model-shaped, booleans, counts, timestamp presence, and raw-ID presence flags; unknown future methods remain unsupported count-only.
  - Preserved privacy boundaries by excluding raw IDs, paths, URLs, commands, file names, query strings, warning/error/message text, deltas, reasoning text, transcripts, summaries, descriptions, schemas, prompts, payloads, tool output, response/request bodies, auth values, account/user identifiers, and arbitrary unknown values.
  - Added focused sanitizer tests for all new Codex 0.140 notification families plus representative lifecycle, hook, app-list, MCP, Windows, streaming, process, file-change, approval, account-login, and server-request privacy boundaries.
  - Next recommended item: `Add local source freshness and capture coverage diagnostics`.

- Use app-server `account/usage/read` for account-token comparison
  - Added sanitized decoding for generated `GetAccountTokenUsageResponse` fields: lifetime tokens, peak daily tokens, bounded daily buckets, longest running turn seconds, current streak days, and longest streak days.
  - Made Settings Data account-token comparison prefer `account/usage/read` through the app-server request pipeline, keeping `/wham/profiles/me` only as a compatibility fallback.
  - Kept server/account token metrics separate from local captured token totals and updated Settings copy/rows to label the server account source clearly.
  - Preserved privacy boundaries by storing only bounded safe metrics and no auth tokens, account IDs, user IDs, emails, profile identity, raw app-server responses, or raw backend responses.
