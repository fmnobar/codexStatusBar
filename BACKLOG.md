...
- Add generated app-server surface drift check
  ...
  - Added a deterministic method-name-only accepted fixture in sanitizer tests, sourced from `ServerNotification.ts` generated by Codex 0.140, with 66 notification method names and no payload schemas.
  - Added tests proving the fixture rejects schema-shaped content, all accepted methods are covered or skipped, synthetic future methods report drift, and unknown runtime methods remain unsupported count-only.
  - Preserved the specialized skipped diagnostic paths for `account/rateLimits/updated`, `remoteControl/status/changed`, and `thread/tokenUsage/updated`.
  - Preserved privacy boundaries by recording method names only and excluding generated payload schemas, raw app-server payloads, prompts/messages/tool content, auth/account/user identifiers, raw thread/turn IDs, URLs, local paths, descriptions, and arbitrary values.
  - Next recommended item: `Tighten source-health update metadata and redacted diagnostics paths`.
...
Next item to plan: `Tighten source-health update metadata and redacted diagnostics paths`.
